...
"""Module for converting ONNX models to QNN format."""
...
from ..core.utils import DEBUG_QNN, run, ONNX_INSTALLED
...
def onnx_to_qnn(
    onnx_file: str,
    qnn_onnx_convert_kwargs: str | List = "",
    qnn_model_lib_generator_kwargs: str | List = "",
    qnn_context_binary_generator_kwargs: str | List = "",
    output: str = None,
) -> QnnModel:
    """
    Convert onnx model to qnn model.

    Args:
        onnx_file: onnx model to convert.
    ...
    os.chdir(temp_dir)
    copied_onnx_file = os.path.join(temp_dir, "model.onnx")
    run(f"cp {onnx_file} {copied_onnx_file}")
    onnx_file = copied_onnx_file

    # infer input_shape and input_names from onnx
    input_names = []
    input_shape_list = []
    input_shape_str = [] 
    ...
    for idx, _input in enumerate(model.graph.input):
        input_info = MessageToDict(_input)
        input_shape = [int(item["dimValue"])
                       for item in input_info["type"]["tensorType"]["shape"]["dim"]]
        input_names.append(input_info["name"])
        input_shape_list.append(input_shape)
        input_shape_str.append(",".join([str(s) for s in input_shape]))

    so_target = "x86_64-linux-clang"
    htp_backend = f"{QNN_SDK_ROOT}/lib/x86_64-linux-clang/libQnnHtp.so"

    # qnn-pytorch-converter
    if isinstance(qnn_onnx_convert_kwargs, list):
        kwargs = " ".join(qnn_onnx_convert_kwargs)
    else:
        kwargs = qnn_onnx_convert_kwargs
    if not DEBUG_QNN:
        kwargs += " 2>/dev/null"
    cmd = (
        f"qnn-onnx-converter --input_network {onnx_file} --float_bitwidth 16 "
        f"--input_dim '{input_names[0]}' {input_shape_str[0]} {kwargs}"
    )
    run(cmd)
